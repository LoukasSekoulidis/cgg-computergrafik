...
public class PolishedMaterial implements Material{
    ...
    public PolishedMaterial(Color albedo, boolean isMatt, double mattness){
        this.albedo = albedo;
        this.isMatt = isMatt;
    ...
    public Ray scatteredRay(Ray incomingRay, Hit hitPoint) {
        Direction reflectedDir = (subtract(incomingRay.d, multiply(hitPoint.n, dotProduct(incomingRay.d, hitPoint.n) * 2)));
        if(isMatt){
            double subtractor = mattness/2;
            reflectedDir = add(reflectedDir, direction(random()*mattness - subtractor, random()*mattness - subtractor, random()*mattness - subtractor));
        }
